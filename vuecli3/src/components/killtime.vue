<template>
  <div class="killTime">
    <p v-if="msTime.show">
      ...
      <!-- v-if="msTime.day > 0" -->
      <span class="day">{{msTime.day}}</span><i>{{dayTxt}}</i>
      <span class="hour">{{msTime.hour}}</span><i>{{hourTxt}}</i>
      <span class="minute">{{msTime.minutes}}</span><i>{{minutesTxt}}</i>
      <span class="second">{{msTime.seconds}}</span><i>{{secondsTxt}}</i>
    </p>
  ...
  export default {
    ...
    watch: {
      currentTime: function (val, oldval){
        clearTimeout(this.timer) // 新增清除定时器
        this.gogogo();
      }
    },
    props: {
